...
if not exist "resources\icon.ico" (
    ...
    if exist "resources\generate_icon.py" (
...
rem Keep .spec files — don't delete
...
pyinstaller scripts\DiskCleaner.spec --clean --noconfirm
...
if exist "dist\DiskCleaner.exe" (
    if exist "resources\icon.ico" (
        copy /Y "resources\icon.ico" "dist\DiskCleaner.ico" >nul 2>&1
    )
)
