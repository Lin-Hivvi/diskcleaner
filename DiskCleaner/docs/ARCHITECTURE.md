...
│  │           独立子模块                                     │       │
│  │  config.py    — 常量 + 权限                             │       │
│  │  log_manager.py — 日志系统（单例）                      │       │
│  │  app_cleaner.py — 应用软件缓存（独立发现引擎）           │       │
│  │  i18n.py      — 国际化系统                              │       │
│  └──────────────────────────────────────────────────────┘       │
└──────────────────────────────────────────────────────────────────┘
```
...
│   ├── app_cleaner.py           # 应用软件缓存清理（微信/QQ/钉钉等）
│   └── i18n.py                  # 国际化模块（中英双语）
├── resources/                   # 资源与工具
│   ├── icon.ico                 # 程序图标
...
├── scripts/                     # 构建脚本
│   ├── build.bat                # 一键打包脚本
│   └── DiskCleaner.spec         # PyInstaller 配置文件
├── dist/                        # 构建输出
│   └── DiskCleaner.exe          # 可执行文件（~13 MB）
├── .claude/                     # Claude AI 配置
└── .gitignore                   # Git 忽略规则
```

> **开发运行：** `python -m src.disk_cleaner`（从项目根目录执行）
> **构建打包：** 双击 `scripts\build.bat` 或运行 `python scripts/build.bat`
...
┌────────────────────┼────────────────────────────────┐
│                    │                                │
│  clean_tasks.py    │  tray.py       log_manager.py │
│  ─────────────    │  ───────       ────────────── │
│  CleanTask (基类)  │  WindowsTray    LogManager    │
│  ├─ TempFilesTask  │  Icon          (单例)         │
│  ├─ RecycleBinTask │                每日轮转       │
│  ├─ BrowserCache   │                线程安全        │
│  ├─ OldDownloads   │                               │
│  ├─ RecentFiles    │  config.py     utils.py       │
│  └─ SystemLogs     │  ─────────     ────────       │
│                    │  常量/权限      格式化/文件     │
│  app_cleaner.py    │  CSIDL/提权   操作/回收站      │
│  ─────────────     │                               │
│  AppSoftwareTask   │  i18n.py                      │
│  ├─ SoftwareCategory│  ───────                     │
│  │  └─ CacheItem    │  中英双语                      │
│  └─ 自动发现注册表   │  JSON 语言包                  │
└────────────────────────────────────────────────────┘
